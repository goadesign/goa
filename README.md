...
```
The context data structure contains the id of the bottle to be shown declared as an `int` since
that's the type specified in the DSL. It also contains an anonymous field which gives access to the
raw underlying request and response states (including access to the `http.Request` and
`http.ResponseWriter` objects. The goa context data structure also implements the golang
[`context.Context`](https://godoc.org/golang.org/x/net/context) interface which makes it possible to
carry deadlines and cancelation signals for example across different goroutines involved in handling
the request. The built-in `Timeout` middleware takes advantage of this.
