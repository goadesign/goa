...
func NewV2(root *http.RootExpr) (*V2, error) {
	if root == nil {
		return nil, nil
	}
...
func hasAbsoluteRoutes(root *http.RootExpr) bool {
...
func paramsFromExpr(params *design.MappedAttributeExpr, path string) ([]*Parameter, error) {
	...
	var (
		res       []*Parameter
		wildcards = http.ExtractWildcards(path)
		i         = 0
	)
...
func paramsFromHeaders(endpoint *http.EndpointExpr) []*Parameter {
	params := []*Parameter{}
	var (
...
func responseSpecFromExpr(s *V2, root *http.RootExpr, r *http.HTTPResponseExpr) (*Response, error) {
	var schema *Schema
...
func buildPathFromFileServer(s *V2, root *http.RootExpr, fs *http.FileServerExpr) error {
	wcs := http.ExtractWildcards(fs.RequestPath)
	var param []*Parameter
	if len(wcs) > 0 {
		param = []*Parameter{{
			In:          "path",
	...
	key := http.WildcardRegex.ReplaceAllStringFunc(
		fs.RequestPath,
...
func buildPathFromExpr(s *V2, root *http.RootExpr, route *http.RouteExpr, basePath string) error {
	...
	bp := http.WildcardRegex.ReplaceAllStringFunc(
		basePath,
		func(w string) string {
			return fmt.Sprintf("/{%s}", w[2:])
		},
