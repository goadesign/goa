...
func APISchema(api *design.APIExpr, r *http.RootExpr) *Schema {
...
func GenerateServiceDefinition(api *design.APIExpr, res *http.ServiceExpr) {
	s := NewSchema()
...
func toSchemaHref(r *http.RouteExpr) string {
	params := r.Params()
	args := make([]interface{}, len(params))
	for i, p := range params {
		args[i] = fmt.Sprintf("/{%s}", p)
	}
	tmpl := http.WildcardRegex.ReplaceAllLiteralString(r.FullPath(), "%s")
