...
import (
	...
	. "goa.design/goa.v2/codegen/http/testing"
...
func TestPayloadConstructor(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.Name, func(t *testing.T) {
			RunHTTPDSL(t, c.DSL)
			if len(http.Root.HTTPServices) != 1 {
				t.Fatalf("got %d file(s), expected 1", len(http.Root.HTTPServices))
			}
			fs := serverType(http.Root.HTTPServices[0], make(map[string]struct{}))
