...
func NewUnionTypeID(union *expr.Union) UnionTypeID {
	...
	writeUnionTypeID(&key, union, make(map[*expr.Object]int), make(map[*expr.Union]int))
...
func writeUnionTypeID(key *strings.Builder, union *expr.Union, objects map[*expr.Object]int, unions map[*expr.Union]int) {
	...
	for _, value := range union.Values {
		...
		writeUnionAttributeID(key, value.Attribute, objects, unions)
	}
}

// writeUnionAttributeID appends the generated Go identity of an attribute.
func writeUnionAttributeID(key *strings.Builder, att *expr.AttributeExpr, objects map[*expr.Object]int, unions map[*expr.Union]int) {
	...
	case expr.UserType:
		...
		writeUnionIDPart(key, actual.Hash())
	...
	case *expr.Array:
		...
		writeUnionAttributeID(key, actual.ElemType, objects, unions)
	case *expr.Map:
		writeUnionIDPart(key, "map")
		writeUnionAttributeID(key, actual.KeyType, objects, unions)
		writeUnionAttributeID(key, actual.ElemType, objects, unions)
	case *expr.Object:
		writeUnionObjectID(key, att, actual, objects, unions)
	case *expr.Union:
		writeUnionTypeID(key, actual, objects, unions)
	case expr.CompositeExpr:
		writeUnionAttributeID(key, actual.Attribute(), objects, unions)
	default:
...
func writeUnionObjectID(key *strings.Builder, parent *expr.AttributeExpr, object *expr.Object, objects map[*expr.Object]int, unions map[*expr.Union]int) {
	...
	for _, field := range *object {
		...
		writeUnionAttributeID(key, field.Attribute, objects, unions)
	}
}
