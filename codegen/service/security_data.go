...
func BuildSchemeData(s *expr.SchemeExpr, m *expr.MethodExpr) *SchemeData {
	...
	if s.Kind == expr.BasicAuthKind {
		...
		return &SchemeData{
			...
			UsernameIsAlias:  expr.IsAlias(m.Payload.Find(userAtt).Type),
			...
			PasswordIsAlias:  expr.IsAlias(m.Payload.Find(passAtt).Type),
	...
	data := &SchemeData{
		...
		CredIsAlias:  expr.IsAlias(m.Payload.Find(keyAtt).Type),
