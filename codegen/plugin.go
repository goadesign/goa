...
type (
	// PlanFunc declares generated package types before the generation is frozen.
	// Planning functions must not render files.
	PlanFunc func(*Generation) error

	// GenerateFunc makes it possible to modify the files generated by the
	// goa code generators and other plugins. It receives the frozen generation
	// used by core generators and the files produced by preceding callbacks.
	GenerateFunc func(*Generation, []*File) ([]*File, error)
	...
	plugin struct {
		...
		PrepareFunc
		// PlanFunc is the plugin declaration planning function.
		PlanFunc
...
// given command.
func RegisterPlugin(name string, cmd string, pre PrepareFunc, plan PlanFunc, generate GenerateFunc) {
	np := &plugin{name: name, PrepareFunc: pre, PlanFunc: plan, GenerateFunc: generate, cmd: cmd}
	var inserted bool
...
func RegisterPluginFirst(name string, cmd string, pre PrepareFunc, plan PlanFunc, generate GenerateFunc) {
	np := &plugin{name: name, PrepareFunc: pre, PlanFunc: plan, GenerateFunc: generate, cmd: cmd, first: true}
	var inserted bool
	for i, plgn := range plugins {
...
// using this, the plugins will be sorted alphabetically by their names. If two
// plugins have same names, then they are sorted by registration order.
func RegisterPluginLast(name string, cmd string, pre PrepareFunc, plan PlanFunc, generate GenerateFunc) {
	np := &plugin{name: name, PrepareFunc: pre, PlanFunc: plan, GenerateFunc: generate, cmd: cmd, last: true}
	var inserted bool
...
// RunPluginsPlan executes plugin planning functions in registration order.
func RunPluginsPlan(cmd string, generation *Generation) error {
	for _, plugin := range plugins {
		if plugin.cmd != cmd || plugin.PlanFunc == nil {
			continue
		}
		if err := plugin.PlanFunc(generation); err != nil {
			return err
		}
	}
	return nil
...
// they were registered.
func RunPlugins(cmd string, generation *Generation, genfiles []*File) ([]*File, error) {
	for _, plugin := range plugins {
		if plugin.cmd != cmd {
			continue
		}
		gs, err := plugin.GenerateFunc(generation, genfiles)
		if err != nil {
			return nil, err
		}
		genfiles = gs
