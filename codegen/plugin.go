...
type (
	...
	// PreGenerateFunc makes it possible to modify the design roots before
	// the files being generated by the goa code generators or other plugins.
	PreGenerateFunc func(genpkg string, roots []eval.Root) error

	// plugin is a plugin that has been registered with a given command.
	plugin struct {
		// GenerateFunc is the plugin pre generator function.
		PreGenerateFunc
...
// given command.
func RegisterPlugin(name string, cmd string, p GenerateFunc, pre PreGenerateFunc) {
	np := &plugin{name: name, GenerateFunc: p, cmd: cmd, PreGenerateFunc: pre}
	var inserted bool
...
func RegisterPluginFirst(name string, cmd string, p GenerateFunc, pre PreGenerateFunc) {
	np := &plugin{name: name, GenerateFunc: p, cmd: cmd, first: true, PreGenerateFunc: pre}
	var inserted bool
	for i, plgn := range plugins {
...
// using this, the plugins will be sorted alphabetically by their names. If two
// plugins have same names, then they are sorted by registration order.
func RegisterPluginLast(name string, cmd string, p GenerateFunc, pre PreGenerateFunc) {
	np := &plugin{name: name, GenerateFunc: p, cmd: cmd, last: true, PreGenerateFunc: pre}
	var inserted bool
...
// RunPluginsPreGenerate executes the plugins pre generation functions in the order
// they were registered.
func RunPluginsPreGenerate(cmd, genpkg string, roots []eval.Root) error {
	for _, plugin := range plugins {
		if plugin.cmd != cmd {
			continue
		}
		if plugin.PreGenerateFunc != nil {
			err := plugin.PreGenerateFunc(genpkg, roots)
			if err != nil {
				return err
			}
		}
