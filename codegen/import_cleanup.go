...
// It looks for qualified identifiers (pkg.Name), type references, and
...
func detectUsedImports(file *ast.File, imports map[string]*importInfo) {
	ast.Inspect(file, func(n ast.Node) bool {
		...
		case *ast.Ident:
			// Handle cases where package name is used directly (rare)
			if imp, exists := imports[x.Name]; exists {
				// Only mark as used if this identifier is not bound to a local object
				// to avoid false positives when a local variable has the same name
				if x.Obj == nil {
					imp.used = true
				}
			}
