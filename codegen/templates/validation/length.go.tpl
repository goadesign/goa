...
if {{ if .string }}{{ .utf8 }}.RuneCountInString({{ $target }}){{ else }}len({{ $target }}){{ end }} {{ if .isMinLength }}<{{ else }}>{{ end }} {{ if .isMinLength }}{{ .minLength }}{{ else }}{{ .maxLength }}{{ end }} {
        err = {{ .goa }}.MergeErrors(err, {{ .goa }}.InvalidLengthError({{ validationPath .context }}, {{ $target }}, {{ if .string }}{{ .utf8 }}.RuneCountInString({{ $target }}){{ else }}len({{ $target }}){{ end }}, {{ if .isMinLength }}{{ .minLength }}, true{{ else }}{{ .maxLength }}, false{{ end }}))
}{{- if and .isPointer .string }}
}
{{- end }}
