...
secure goa endpoint. It supports the following security schemes:
  ...
  - Bearer security using bearer tokens.
...
type (
	...
	// BearerScheme represents an HTTP Bearer security scheme.
	// It consists of a raw bearer token which is used in authentication.
	// The token may be opaque, a JWT, or any other format accepted by the
	...
	// JWTScheme represents a JWT bearer security scheme.
	// It uses the same HTTP Bearer wire format as BearerScheme, but is
	// generated by JWTSecurity for designs that intentionally model JWT
	// tokens in their public service API.
	JWTScheme struct {
		// Name is the scheme name defined in the design.
		Name string
		// Scopes holds a list of scopes for the scheme.
	...
	// AuthBearerFunc is the function type that implements the HTTP bearer
	// scheme of using a bearer token.
	AuthBearerFunc func(ctx context.Context, token string, s *BearerScheme) (context.Context, error)
...
// Validate returns a non-nil error if scopes does not contain all of
// Bearer scheme's required scopes.
func (s *BearerScheme) Validate(scopes []string) error {
	return validateScopes(s.RequiredScopes, scopes)
}
