/*
Package dslexec implements a DSL engine for executing arbitrary Go DSLs.

DSLs executed via dslexec consist of package functions that build up expressions upon execution.
...
root expression structs, the only requirement is that they implement the dslexec package Expression
...
instance. This is where the RunDSL function of package dslexec comes in.
...
field to the iterator:
    ...
    func (se *ServiceExpression) IterateSets(it dslexec.SetIterator) {
        it(se.Endpoints)
    }
...
The dslexec package exposes functions that the implementation of the DSL can take advantage of to
