...
{{tabs $depth}}	err = err.Merge(goa.InvalidEnumValueError(` + "`" + `{{.context}}` + "`" + `, {{.targetVal}}, {{slice .values}}))
...
{{tabs $depth}}	err = err.Merge(goa.InvalidPatternError(` + "`" + `{{.context}}` + "`" + `, {{.targetVal}}, ` + "`{{.pattern}}`" + `))
...
{{tabs $depth}}		err = err.Merge(goa.InvalidFormatError(` + "`" + `{{.context}}` + "`" + `, {{.targetVal}}, {{constant .format}}, err2))
{{if .isPointer}}{{tabs $depth}}}
{{end}}{{tabs .depth}}}`
...
{{tabs $depth}}	err = err.Merge(goa.InvalidRangeError(` + "`" + `{{.context}}` + "`" + `, {{.targetVal}}, {{if .isMin}}{{.min}}, true{{else}}{{.max}}, false{{end}}))
{{if .isPointer}}{{tabs $depth}}}
{{end}}{{tabs .depth}}}`
...
{{tabs $depth}}	err = err.Merge(goa.InvalidLengthError(` + "`" + `{{.context}}` + "`" + `, {{$target}}, len({{$target}}), {{if .isMinLength}}{{.minLength}}, true{{else}}{{.maxLength}}, false{{end}}))
{{if .isPointer}}{{tabs $depth}}}
{{end}}{{tabs .depth}}}`
...
{{tabs $.depth}}	err = err.Merge(goa.MissingAttributeError(` + "`" + `{{$.context}}` + "`" + `, "{{$r}}"))
{{tabs $.depth}}}
