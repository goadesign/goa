...
func Tag(name, value string) {
	res, ok := eval.Current().(*http.HTTPResponseExpr)
	if !ok {
		eval.IncompatibleDSL()
		return
	}
...
func httpError(n string, p eval.Expression, args ...interface{}) *http.ErrorExpr {
	if len(args) == 0 {
