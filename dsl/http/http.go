...
func HTTP(fn func()) {
	...
	case *http.RootExpr:
		eval.Execute(fn, http.Root)
	case *design.ServiceExpr:
		res := http.Root.ServiceFor(actual)
		res.DSLFunc = fn
	case *design.MethodExpr:
		res := http.Root.ServiceFor(actual.Service)
...
func Consumes(args ...string) {
	...
	case *http.RootExpr:
		def.Consumes = append(http.Root.Consumes, args...)
	default:
		eval.IncompatibleDSL()
	}
}
...
func Produces(args ...string) {
	switch def := eval.Current().(type) {
	case *http.RootExpr:
		def.Produces = append(http.Root.Produces, args...)
	default:
		eval.IncompatibleDSL()
	}
}
...
func Path(val string) {
	...
	case *design.APIExpr:
		http.Root.Path = val
	case *http.ServiceExpr:
		def.Path = val
		if !strings.HasPrefix(val, "//") {
			awcs := http.ExtractWildcards(http.Root.Path)
			wcs := http.ExtractWildcards(val)
...
func Headers(args interface{}) {
	h, ok := eval.Current().(http.HeaderHolder)
	if !ok {
		eval.IncompatibleDSL()
		return
	}
...
func Params(args interface{}) {
	h, ok := eval.Current().(http.ParamHolder)
	if !ok {
		eval.IncompatibleDSL()
		return
	}
...
func Body(args ...interface{}) {
	...
	case *http.ErrorExpr:
		...
		setter = func(att *design.AttributeExpr) {
			if e.Response == nil {
				e.Response = &http.HTTPResponseExpr{}
			}
			e.Response.Body = att
		}
		kind = "Error"
	...
	case *http.HTTPResponseExpr:
		ref = e.Parent.(*http.EndpointExpr).MethodExpr.Result
		setter = func(att *design.AttributeExpr) {
			e.Body = att
		}
		kind = "Response"
...
func Parent(name string) {
	r, ok := eval.Current().(*http.ServiceExpr)
	if !ok {
		eval.IncompatibleDSL()
		return
	}
