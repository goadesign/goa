...
# How do I upgrade services that still have clients using the pre-envelope stream protocol?

Set `Meta("grpc:stream:compat", "v1")` on the method (or the service or API to
cover all methods). Generated servers then accept both protocols: clients
generated by current goa versions declare the envelope protocol through the
`goa-stream-protocol` request metadata key while legacy clients send no such
key and keep carrying the one-shot payload in request metadata. Upgrade all
servers first, then the clients, and finally remove the meta to drop the
compatibility code. The legacy protocol only encodes primitives and arrays of
primitives so the meta requires the method payload to be limited to such
attributes.
