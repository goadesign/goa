...
func Action(name string, dsl func()) {
	if r, ok := resourceDefinition(true); ok {
		...
		if !engine.ExecuteDSL(dsl, action) {
			return
		}
...
func Headers(dsl func()) {
	if a, ok := actionDefinition(false); ok {
		...
		if engine.ExecuteDSL(dsl, headers) {
			a.Headers = headers
		}
		...
		if r.Headers != nil {
			engine.ReportError("headers already defined")
			return
		}
		...
		default:
			engine.ReportError("invalid use of Response or ResponseTemplate")
		}
		if engine.ExecuteDSL(dsl, h) {
			r.Headers = h
		}
	}
}
...
func Payload(p interface{}, dsls ...func()) {
	if len(dsls) > 1 {
		engine.ReportError("too many arguments given to Payload")
		return
	}
	if a, ok := actionDefinition(true); ok {
		...
		case string:
			...
			if !ok {
				engine.ReportError("unknown payload type %s", actual)
			}
		...
		if len(dsls) == 1 {
			if dsl != nil {
				engine.ReportError("invalid arguments in Payload call, must be (type), (dsl) or (type, dsl)")
			}
			dsl = dsls[0]
		}
		if dsl != nil {
			engine.ExecuteDSL(dsl, att)
		}
