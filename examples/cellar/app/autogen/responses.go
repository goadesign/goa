...
var RespondBadRequest = func(err error, ctx *goa.Context) error {
	return ctx.RespondBadRequest(err.Error())
}

// Default bad response handler.
// Invoked when an invalid response is sent.
// Returns status code 500 and error message in body.
var RespondBadResponse = func(err error, ctx *goa.Context) error {
	return ctx.RespondBadResponse(err.Error())
}
